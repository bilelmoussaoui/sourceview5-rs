...
impl Buffer {
    pub fn new(table: Option<&gtk::TextTagTable>) -> Buffer {
        ...
        unsafe {
            from_glib_full(gtk_source_sys::gtk_source_buffer_new(table.to_glib_none().0))
        }
    }
...
impl<O: IsA<Buffer>> BufferExt for O {
    ...
    #[cfg(any(feature = "v5_0", feature = "dox"))]
    fn connect_cursor_moved<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn cursor_moved_trampoline<P, F: Fn(&P) + 'static>(this: *mut gtk_source_sys::GtkSourceBuffer, f: glib_sys::gpointer)
            where P: IsA<Buffer>
        {
            let f: &F = &*(f as *const F);
